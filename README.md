...
New commissions stay PENDING for `commissionHoldDays` (default 30) so Stripe chargebacks can claw back. A cron then matures due rows and pays only APPROVED commissions (`maturesAt` ≤ now) via the PayPal Payouts API on the program payday (monthly default: the 15th; tiers can override). Each cron run pays at most a few affiliates (drip size, default 2) so a mass pay does not hit your PayPal balance.
